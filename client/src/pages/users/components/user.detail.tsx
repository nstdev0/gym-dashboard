  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const response: UserSchema = await apiFetch(`/users/${id}`, "GET", null, {Authorization: `Bearer ${token}`});
        if (!response) {
                    ...
                    <Card>
                        <CardHeader className="border-b border-border/40 bg-muted/20">
                            <div className="flex justify-between items-start">
                                ...
                                <div className={`px-2 py-1 rounded-full text-xs font-semibold ${user.isActive ? "bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-400" : "bg-gray-100 text-gray-600"}`}>
                        ...
                        <CardFooter className="bg-muted/10 text-xs text-muted-foreground border-t border-border/40 py-3 flex justify-between">
                            ...
                            <span>Registro: {new Date(user?.createdAt).toLocaleDateString("es-PE")}</span>
                        </CardFooter>
                    </Card>
                </div>
            )}
